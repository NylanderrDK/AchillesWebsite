<?php
  ...
  $latestSql = mysqli_query($conn, "SELECT * FROM uploads WHERE category = 'police' ORDER BY id DESC LIMIT 3");
  $popularSql = mysqli_query($conn, "SELECT * FROM uploads WHERE category = 'police' ORDER BY downloads DESC LIMIT 3");
?>
  ...
  <main>
    ...
    <div class="hero" style="background-image:url('img/PoliceVehicles.webp');background-repeat:no-repeat;background-size:cover;background-position:center;height:40vh;">
      <div class="h-100 d-flex justify-content-center align-items-center text-white text-center">
        ...
        <span style="font-weight:200;font-size:2rem;">Politi</span></h1>
      </div>
    </div>
    <!-- Hero END -->
